<?php
  ...
  class Questions extends Connection{
    ...
    public function soal_code(){
          $num       = '';
          $perfix    = 'FQT';
          $sql       = "SELECT MAX(id) AS kode FROM final_exam";
    ...
    /*Get All*/
    public function getAllsoalid($id){
      $data=array();
       $sql    = "SELECT *
                  FROM
                    final_exam WHERE id='$id'";
    ...
    /*Get by excerpt*/
    public function getAllsoal($excerpt){
    	$data=array();
    	 $sql    = "SELECT *
                  FROM
                    final_exam WHERE excerpt='$excerpt'";
        $exe  = $this->conn->query($sql);
        $row  = $exe->num_rows;
        if($row>0){
            if($exe){
    ...
    /*Get*/
    public function get_soal($id, $excerpt){
       $data =array();
       $sql    = "SELECT
                    *
                  FROM
                    final_exam
                  WHERE lid='$id' AND excerpt='$excerpt'";
    ...
    public function update_soal($id){
        ...
        $mid       = $this->real_escape($_POST['lid']);
        ...
        $urld       = 'http://localhost/baqi/assets/imgsoal/';
        $qremove   = "SELECT * FROM final_exam WHERE id='$id'";
    ...
    public function insert_soal(){
        ...
        if(($img_response=='uploaded') && ($crt_img=='support') && ($img_responseutm=='uploaded' || $img_responseutm=='no image')){
            $sql    = "INSERT INTO final_exam(id, question, type, lid,excerpt, options, image, correct_option, updated) 
                       VALUES ('$id','$question','$type','$mid','$excerpt','$newoptions','$url_univ','$newCorrectOption','')";
            $exe    =  $this->conn->query($sql);
            if($exe){
              $response=array( 
                               'status' => 201,
    ...
    public function delete_soal($id){
        ...
        $sql   = "DELETE FROM final_exam WHERE id='$id'";
        $exe   = $this->conn->query($sql);
        if($exe){
           $response=array(
                             'status' => 200,
