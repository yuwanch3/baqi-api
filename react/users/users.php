<?php
  ...
  class Users extends Connection{
    ...
    public function getAllusers(){
    	$data=array();
    	 $sql    = "SELECT id, name, email, phone, username, password, picture, created, updated
                  FROM
                    users";
    ...
    /*Update*/
    public function update_users($id,$name,$email,$phone){
        $id  = $this->real_escape($id);
        $mb_name = $this->real_escape($name);
        $mb_email = $this->real_escape($email);
        $mb_phone  = $this->real_escape($phone); 
    ...
    public function update_photo($id){
        ...
        $url       = $localIP.'/assets/img/';
        $urlold    = 'https://api.pondok-huda.com/assets/img/';
        //Foto
    ...
    /*Insert*/
    public function insert_users($name,$email,$phone,$username,$password,$picture){
        $mb_id  = $this->users_code();
        $mb_name = $this->real_escape($name);
        $mb_email = $this->real_escape($email);
        $mb_phone  = $this->real_escape($phone);
        $mb_username = $this->real_escape($username);
        $mb_password = $this->real_escape($password);
        $mb_picture = $this->real_escape($picture);
        

          $sql    = "INSERT INTO users(id, name, email, phone, username, password, picture, updated)
                   VALUES ('$mb_id','$mb_name','$mb_email','$mb_phone','$mb_username','$mb_password','$picture','')";
          $exe    = $this->conn->query($sql);
          if($exe){
            $response=array(
                             'status' => 201,
