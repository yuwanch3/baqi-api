<?php
  ...
  class Last extends Connection{
    ...
    public function last_code(){
          $num       = '';
          $perfix    = 'LST';
          $sql       = "SELECT MAX(id) AS kode FROM lastest";
    ...
    public function get_last($uid){
       ...
       $sqlt    = "SELECT
                        ...
                        lastest.sublid, lastest.excerpt, lastest.total, lastest.wrong,
                        lastest.correct, lastest.current, lastest.status,
                        lastest.dated, sub_levels.name, sub_levels.level
                      FROM
                        lastest INNER JOIN
    ...
    /*Update*/
    public function update_last($id, $uid, $mid, $lid, $sublid, $total, $excerpt, $correct, $wrong, $current, $status){
        $id  = $this->real_escape($id);
        $mid = $this->real_escape($mid);
        $uid = $this->real_escape($uid);
        $lid = $this->real_escape($lid);
        $sublid = $this->real_escape($sublid);
        $total   = $this->real_escape($total);
        $wrong   = $this->real_escape($wrong);
        $correct = $this->real_escape($correct);
        $excerpt = $this->real_escape($excerpt);
        $current = $this->real_escape($current);
    ...
    /*Insert*/
    public function insert_last($uid, $mid, $lid, $sublid, $total, $excerpt, $correct, $wrong, $current, $status){
        $id  = $this->last_code();
        $mid = $this->real_escape($mid);
        $uid = $this->real_escape($uid);
        $lid = $this->real_escape($lid);
        ...
        $status  = $this->real_escape($status);
          ...
          $sql    = "INSERT INTO lastest(id, mid, uid, lid, sublid, excerpt, total, wrong, correct, current, status, dated)
                   VALUES ('$id','$mid','$uid','$lid','$sublid','$excerpt','$total','$wrong','$correct','$current','$status','')";
          $exe    = $this->conn->query($sql);
          if($exe){
            $response=array(
                             'status' => 201,
